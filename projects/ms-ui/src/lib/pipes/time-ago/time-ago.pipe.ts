...
export class TimeAgoPipe implements PipeTransform, OnDestroy {
  ...
  transform(value: Date | string | null | undefined, live = false): string {
    this.refreshTick();
    ...
    const relativeTime = RELATIVE_TIME_UNITS.find(({ limit }) => Math.abs(difference) < limit);

    if (!relativeTime) {
      return '';
    }

    return this.formatter.format(Math.round(difference / relativeTime.size), relativeTime.unit);
  }
  ...
  private parseTimestamp(value: Date | string | null | undefined): number | undefined {
