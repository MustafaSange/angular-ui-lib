...
package under `dist/ms-ui`, and creates a versioned snapshot such as `dist/ms-ui-0.0.2`.
...
```bash
npm install ../ui-lib/dist/ms-ui-0.0.2
```
...
{
  "dependencies": {
    "ms-ui": "file:../ui-lib/dist/ms-ui-0.0.2"
  }
}
```

Replace `0.0.2` with the version that should be installed, and adjust the relative path when the
...
For example, after a patch release changes `0.0.2` to `0.0.3`, install its new versioned folder:

```bash
npm install ../ui-lib/dist/ms-ui-0.0.3
```
