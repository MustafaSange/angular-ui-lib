---
name: git-publish-enhancement
description: Publish all current repository changes on a new enhancement branch with a meaningful branch name, commit subject and descriptive body, then push it with an upstream. Use when the user asks to create an enhancement or feature branch, stage all changes, commit them with a proper title/description or message, and push the branch.
---

# Publish an Enhancement Branch
...
4. Derive a concise lowercase kebab-case branch summary from the dominant change. Create `enhancement/<summary>` from the current commit. If that branch already exists locally or remotely, choose a clear non-conflicting suffix unless reusing it is explicitly requested.
...
- Prefer branch names such as `enhancement/move-showcase-code-to-shared`.
- Prefer Conventional Commit subjects when they fit, such as `feat: move showcase code outside ui library`.
